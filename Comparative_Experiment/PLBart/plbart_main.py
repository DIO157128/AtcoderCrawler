...
from transformers import (AdamW, get_linear_schedule_with_warmup,
                          T5ForConditionalGeneration, RobertaTokenizer, T5Config, PLBartTokenizer,
                          PLBartForConditionalGeneration, PLBartConfig)
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    for batch in bar:
        ...
        for i in range(args.num_beams):
            ...
            tem.append(raw_pred.replace("__java__"," java "))
...
def main():
    ...
    if args.do_test:
        checkpoint_prefix = f'checkpoint-best-loss/{args.model_name}'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
        model = PLBartForConditionalGeneration.from_pretrained(output_dir, config = config)
        model.to(args.device)
