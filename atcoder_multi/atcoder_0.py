...
def extra_page(path,name):
    ...
    submission_list = []
    driver = signIn()
    for index in tqdm.tqdm(range(0,len(hrefs),2)):
        href = hrefs[index]
...
if __name__ == "__main__":
    ...
    # count = 0
    # for href in hrefs:
    #     name = "AtCoder_raw_data_{}_{}.csv".format(pr ocess,count)
    #     count+=1
    #     extra_contest(href,name)
    #3 爬起失败的页面
    extra_page("example.log".format(process),"AtCoder_raw_data_{}_{}.csv".format(process,2))
